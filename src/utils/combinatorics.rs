...
pub fn cartesian_product(sizes: &[usize]) -> CartesianProduct {
    CartesianProduct { sizes: sizes.to_vec(), idxs: vec![0; sizes.len()], done: false }
}
...
mod permutations_tests {
    ...
    fn test_permutations_nonempty() {
        ...
        assert_eq!(perms, vec![vec![0, 1, 2], vec![1, 0, 2], vec![2, 0, 1], vec![0, 2, 1], vec![1, 2, 0], vec![2, 1, 0],])
    }
}
...
mod cartesian_product_tests {
    ...
    fn test_cartesian_product_nonempty() {
        ...
        assert_eq!(prod, vec![vec![0, 0], vec![0, 1], vec![0, 2], vec![1, 0], vec![1, 1], vec![1, 2],]);
    }
}
